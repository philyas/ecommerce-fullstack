/**
 * Hauptseite der Einkaufsliste.
 * Verantwortlich für die Orchestrierung der Shopping-Liste.
 */
...
export function ShoppingListPage() {
  ...
  // Congrats-Modal anzeigen, wenn alle Items erledigt wurden
  useEffect(() => {
  ...
  const handleAddSuccess = (name: string, quantity?: number) => {
    const msg =
      quantity && quantity > 1
    ...
    <div className="flex min-h-0 flex-1 flex-col animate-fade-in">
      ...
      <div className="shrink-0">
        <div className="card p-4 sm:p-6">
          <AddItemForm onAdd={addItem} onSuccess={handleAddSuccess} />
      ...
      <div className="mt-6">
        <div
          className="card flex min-h-[12rem] flex-col overflow-hidden"
          style={{ height: 'min(calc(100vh - 380px), 42rem)' }}
        >
          {/* Delete All Header */}
          {items.length > 0 && !loading && (
            <ListHeader onDeleteAll={() => setShowDeleteAllConfirm(true)} />
          )}
...
function StatusLine({ items, loading, pendingCount, boughtCount }: StatusLineProps) {
...
function ListHeader({ onDeleteAll }: ListHeaderProps) {
  return (
